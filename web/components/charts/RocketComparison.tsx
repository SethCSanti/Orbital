...
export default function RocketComparison({ rockets, comparison, selectedNames, onSelect }: RocketComparisonProps) {
  ...
  return <div className="space-y-6">
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4" aria-label="Rocket selector">
      {rockets.map((rocket) => <label key={rocket.name} className={`flex cursor-pointer items-start gap-3 border p-4 transition-colors ${selectedNames.includes(rocket.name) ? "border-signal bg-panel-raised" : "border-line bg-panel hover:border-signal/60"}`}><input type="checkbox" checked={selectedNames.includes(rocket.name)} onChange={() => onSelect(rocket.name)} className="mt-1 accent-signal" /><span><span className="block font-display text-sm font-semibold text-ink">{rocket.name}</span><span className="mt-1 block text-xs text-muted">{rocket.family} · {rocket.maidenFlight || "Maiden flight unknown"}</span></span></label>)}
    </div>
