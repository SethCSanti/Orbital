...
export default function RocketComparison({ rockets, comparison, selectedNames, onSelect }: RocketComparisonProps) {
  const selected = (comparison ?? rockets).filter((rocket) => selectedNames.includes(rocket.name));
  ...
  return <div className="space-y-6">
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4" aria-label="Rocket selector">
      {rockets.map((rocket) => <label key={rocket.id} className={`flex cursor-pointer items-start gap-3 border p-4 transition-colors ${selectedNames.includes(rocket.name) ? "border-signal bg-panel-raised" : "border-line bg-panel hover:border-signal/60"}`}><input type="checkbox" checked={selectedNames.includes(rocket.name)} onChange={() => onSelect(rocket.name)} className="mt-1 accent-signal" /><span><Link href={`/rockets/${rocket.id}`} prefetch={false} className="block font-display text-sm font-semibold text-ink hover:text-signal">{rocket.name}</Link><span className="mt-1 block text-xs text-muted">{rocket.family} · {rocket.maidenFlight || "Maiden flight unknown"}</span></span></label>)}
    </div>
    {selected.length > 0 && <div className="overflow-x-auto border border-line bg-panel p-5"><div className="flex min-w-[620px] items-end justify-around gap-5 border-b border-line pb-5" aria-label="Rocket silhouettes to scale">{selected.map((rocket) => <div key={rocket.name} className="flex min-w-[120px] flex-1 flex-col items-center justify-end"><span className="mb-3 text-center text-xs text-muted">{rocket.length} m</span><button type="button" onClick={() => onSelect(rocket.name)} className="relative w-14 bg-signal/75 transition-colors hover:bg-signal" style={{ height: `${Math.max(28, rocket.length / maxLength * MAX_HEIGHT)}px` }} aria-label={`Deselect ${rocket.name}`}><span className="absolute -top-3 left-1/2 h-4 w-4 -translate-x-1/2 rounded-full bg-signal-strong" /><span className="absolute inset-x-[-6px] bottom-0 h-3 bg-signal-strong/45" /></button><span className="mt-3 text-center font-display text-sm font-semibold text-ink">{rocket.name}</span></div>)}</div><div className="mt-5 min-w-[620px] overflow-hidden"><table className="w-full border-collapse text-left text-sm"><thead><tr className="border-b border-line text-xs uppercase tracking-[0.12em] text-muted"><th className="p-3">Measure</th>{selected.map((rocket) => <th className="p-3" key={rocket.name}>{rocket.name}</th>)}</tr></thead><tbody>{stats.map(([label, getter]) => <tr key={label} className="border-b border-line/70"><th className="p-3 font-normal text-muted">{label}</th>{selected.map((rocket) => <td className="p-3 text-ink" key={`${label}-${rocket.name}`}>{getter(rocket)}</td>)}</tr>)}</tbody></table></div></div>}
  </div>;
}
