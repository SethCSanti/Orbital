...
const quickLinks = [
  ...
  ["/solar-system", "Solar system", "Explore planetary positions and motion."],
  ["/missions", "Mission archive", "Browse missions and station history."],
  ["/rockets", "Rocket library", "Compare launch vehicles and records."],
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <section className="border-b border-line pb-10 pt-4 sm:pt-10">
        ...
        <p className="mt-5 max-w-2xl text-lg leading-8 text-muted">A practical view of launches, missions, planetary bodies, and the objects passing close to Earth.</p>
      </section>
      <section className="grid gap-px border-x border-b border-line bg-line sm:grid-cols-2" aria-label="Feature shortcuts">
        {quickLinks.map(([href, title, description]) => (
          <Link key={href} href={href} prefetch={false} className="group bg-orbit-950 p-6 transition-colors hover:bg-panel sm:p-8">
