...
export default function RocketsPage() {
  ...
  return <div className="mx-auto max-w-[1500px] space-y-8 px-5 py-8 lg:px-10">
    ...
    {rockets.data && <><RocketComparison rockets={rockets.data.items} comparison={comparison.data} selectedNames={selected} onSelect={toggleRocket} /><Pagination page={rockets.data.page} pageSize={rockets.data.pageSize} total={rockets.data.total} onChange={setPage} /></>}
    ...
    {rockets.data?.items.map((rocket) => <Link key={`detail-${rocket.id}`} href={`/rockets/${rocket.id}`} prefetch={false} className="sr-only">Open details for {rocket.name}</Link>)}
  </div>;
}
