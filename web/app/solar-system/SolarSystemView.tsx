...
export default function SolarSystemView() {
  ...
  const dateKey = new Date(Math.floor(simulatedDate.getTime() / 60_000) * 60_000);
  const query = useQuery({ queryKey: ["solar-system", dateKey.toISOString()], queryFn: () => api.solarSystem.positions(dateKey.toISOString()), staleTime: 60_000 });
  ...
  useEffect(() => { if (isPaused) return; const timer = window.setInterval(() => setSimulatedDate(new Date(simulatedDate.getTime() + timeScale * 86_400_000)), 1000); return () => window.clearInterval(timer); }, [isPaused, simulatedDate, setSimulatedDate, timeScale]);
  ...
  return <div className="mx-auto max-w-[1500px] space-y-8 px-5 py-8 lg:px-10">
    ...
    {query.isLoading && <LoadingState label="Calculating planetary positions" />}{query.error && <ErrorState message={query.error instanceof Error ? query.error.message : "Solar system data unavailable."} onRetry={() => void query.refetch()} />}{query.data && <div className="relative"><SolarSystemCanvas positions={query.data} scaleMode={scaleMode} showOrbits={showOrbits} showLabels={showLabels} selectedPlanet={selectedPlanet} cameraDistance={cameraDistance} onSelect={selectPlanet} /><PlanetInfoPanel name={selectedPlanet} distanceAu={selectedPosition ? Math.sqrt(selectedPosition.x ** 2 + selectedPosition.y ** 2 + selectedPosition.z ** 2) : undefined} orbitalPeriodDays={selectedPosition?.orbitalPeriodDays} onClose={() => selectPlanet("")} /></div>}
  </div>;
}
